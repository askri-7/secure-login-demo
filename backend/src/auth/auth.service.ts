...
export class AuthService {
  ...
  async signUp(
    ...
    const verificationToken = await this.emailVerification.createToken(
      user.id,
      email,
    );
    ...
    try {
      ...
      return {
        message:
          'Account created, but email delivery failed. Please contact support or try signing up again later.',
      };
    }
  ...
  async refresh(
    ...
    return this.prisma.$transaction(async (tx) => {
      //  Only succeed if token is STILL active
      ...
      //  If no rows were updated, someone else won
  ...
  private async handleOAuthLogin(
    ...
    const { user, linked } = await this.prisma.$transaction(async (tx) => {
      ...
      //  Block OAuth linking if local account is locked
      if (existingUser?.lockedUntil && existingUser.lockedUntil > new Date()) {
        const mins = Math.ceil(
          (existingUser.lockedUntil.getTime() - Date.now()) / 60000,
        );
        throw new UnauthorizedException(
          `Too many failed attempts. Account locked for ${mins} minute(s).`,
        );
  ...
  private async handleFailedLogin(
    ...
    const isNewWindow =
      !user.lastFailedLoginAt || user.lastFailedLoginAt < oneMinuteAgo;
