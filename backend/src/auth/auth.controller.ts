...
export class AuthController {
  ...
  private setGoogleAuthRequestCookie(
    res: Response,
  ...
  private setGithubAuthRequestCookie(
    res: Response,
  ...
  async githubLogin(@Res() res: Response) {
    const { url, request } =
      this.githubOAuthService.buildAuthorizationRequest();
  ...
  async githubCallback(
    ...
    const profile = await this.githubOAuthService.completeAuthorization(
      code,
  ...
  async googleLogin(@Res() res: Response) {
    const { url, request } =
      await this.googleOidcService.buildAuthorizationRequest();
  ...
  async googleCallback(@Req() req: Request, @Res() res: Response) {
    ...
    const profile = await this.googleOidcService.completeAuthorization(
      currentUrl,
      authRequest,
    );
  ...
  async verifyEmail(@Query('token') token: string, @Res() res: Response) {
    ...
    try {
      ...
      return res.redirect(
        `${frontendUrl}/verify-email?error=expired_or_invalid`,
      );
    }
  }
}
